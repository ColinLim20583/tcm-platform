...
def init_db():
    ...
    c.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS visual_diagnoses (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            created_at TEXT NOT NULL,
            scan_type TEXT,
            primary_pattern TEXT,
            primary_pattern_zh TEXT,
            constitution_type TEXT,
            confidence_overall INTEGER,
            diagnosis_json TEXT,
            led_to_formulation INTEGER DEFAULT 0
        );
...
def save_formulation(data: dict) -> int:
    ...
    c.execute("""
        INSERT INTO formulations (
            ...
            gross_margin_est, formula_complexity, tags, notes, source
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
...
def save_visual_diagnosis(data: dict) -> int:
    conn = get_connection()
    c = conn.cursor()
    c.execute("""
        INSERT INTO visual_diagnoses (
            created_at, scan_type, primary_pattern, primary_pattern_zh,
            constitution_type, confidence_overall, diagnosis_json
        ) VALUES (?,?,?,?,?,?,?)
    """, (
        datetime.now().isoformat(),
        data.get("scan_type", "full"),
        data.get("primary_pattern", ""),
...
def get_stats():
    ...
    c.execute("SELECT COUNT(*) FROM visual_diagnoses")
    scans = c.fetchone()[0]
    ...
    conn.close()
    return {"total": total, "starred": starred, "scans": scans, "top_patterns": top_patterns}

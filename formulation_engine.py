...
def _safe_json_parse(raw: str) -> dict:
    ...
    # 2) Strip trailing commas before } or ]  (common Claude output issue)
    ...
    # 3) Progressive truncation — find longest valid prefix (handles cut-off responses)
    for i in range(len(cleaned) - 1, max(len(cleaned) - 2000, 0), -1):
        if cleaned[i] in ']}':
            try:
                result = json.loads(cleaned[:i + 1])
...
def generate_formulation(user_input: dict, api_key: str) -> dict:
    ...
    vision_context = user_input.get("vision_context", "")
    ...
    vision_section = ""
    if vision_context:
        vision_section = f"\n=== VISUAL DIAGNOSIS PRE-ASSESSMENT ===\n{vision_context}\n"
...
{{
  ...
  "safety": {{
    ...
    "regulatory_notes": "Singapore HSA notes",
  ...
  "commercial": {{
    ...
    "competitive_gap": "competitive advantage description",
    "channel_recommendation": "recommended channels"
  }},
  "dosage_recommendation": "e.g. 5g twice daily dissolved in warm water",
  "manufacturing_notes": "manufacturing complexity notes",
  "tags": ["Sleep", "Stress"]
}}
